...
export function normalizeTargetCompetitorInput(input: TargetCompetitorInput): TargetCompetitorArtifact {
	...
	const normalizedCompetitors = input.competitors.map((competitor, index) => {
		...
		const competitorNameKey = competitorName.toLocaleLowerCase("en-US");
		...
		if (competitorNames.has(competitorNameKey)) {
		...
		if (competitorIds.has(competitorId)) {
			throw new IntakeInputValidationError(`Duplicate competitor id is not allowed: ${competitorId}`);
		}

		competitorHosts.add(competitorSite.host);
		competitorNames.add(competitorNameKey);
...
function normalizeSite(value: string, fieldLabel: string): IntakeSite {
	...
	const canonicalHost = canonicalizeHost(url.hostname);
	if (canonicalHost.length === 0) {
		throw new IntakeInputValidationError(`Expected ${fieldLabel} to include a hostname.`);
	}

	const origin = `${url.protocol}//${canonicalHost}${url.port.length > 0 ? `:${url.port}` : ""}`;
...
function canonicalizeHost(host: string): string {
	return host.toLocaleLowerCase("en-US").replace(TRAILING_DOT_PATTERN, "");
}
